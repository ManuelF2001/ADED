...
if __name__ == '__main__':
    ...
    # =========================================================================
    # OPTIMIZATION: Caching / Persisting (MEMORY_AND_DISK)
    # Em vez de ler os ficheiros do disco dezenas de vezes para cada loop,
    # guardamos o DataFrame limpo na RAM do cluster.
    # =========================================================================
    print("A forçar a materialização do Cache para MEMORY_AND_DISK...")
    nd = nd.persist(pyspark.StorageLevel.MEMORY_AND_DISK)

    # O .count() forca o Spark a executar o plano físico imediatamente e a
    # guardar os blocos na memória. Sem isto, o Spark seria "preguiçoso".
    cache_count = nd.count()
    print(f"[{cache_count} linhas guardadas em Cache]")
    # =========================================================================

    print("\n\n--- PHYSICAL PLAN (CACHING OPTIMIZATION) ---")
    nd.explain("extended")
    print("--------------------------------------------\n\n")

    cl = ['ARM', 'AMD', 'GPU']

    # LÓGICA DE ESCRITA BASELINE (MANTIDA DE PROPÓSITO)
